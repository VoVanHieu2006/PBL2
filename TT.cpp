...
vector<int> KhoiTaoBangChenDiemXaNhat(int so_diem, const vector<vector<int>>& ban_do_khoang_cach) {
    ...
    // Chèn từng điểm xa nhất vào tour
    while (tour.size() < so_diem - 1) {
        int diem_xa_nhat = -1;
        int khoang_cach_xa_nhat = -1;
        
        // Tìm điểm xa nhất chưa thăm
    ...
    while (da_cai_thien) {
        ...
        for (int i = 0; i < n - 1; ++i) {
            for (int j = i + 1; j < n; ++j) {
                ...
                // Kiểm tra xem có cải thiện được không
...
void PhaRoiLoTrinh(vector<int>& lo_trinh, mt19937& bo_sinh_so_ngau_nhien) {
    ...
    // Kiểm tra các vị trí có trùng nhau không
    if (vi_tri[0] == vi_tri[1] || vi_tri[1] == vi_tri[2] || vi_tri[2] == vi_tri[3]) {
        return;
    }
    
    // Tạo lộ trình mới với các đoạn bị đảo lộn
...
int main() {
    ...
    if (so_tram < NGUONG_CHUYEN_DOI_THUAT_TOAN) {
        ...
        cout << "Thu tu cac tram dung chan (chi so bat dau tu 1):" << endl;
        cout << "Xuat phat -> ";
        
        for (int chi_so_diem : ket_qua.first) {
            cout << chi_so_diem << " -> ";
        }
