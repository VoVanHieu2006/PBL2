...
int main() {
    ...
    int so_hang, so_cot;
#ifndef LOCAL
    // Khi chạy trên OJ, không in prompt
    if (!(cin >> so_hang >> so_cot)) return 0;
#else
    cerr << "Nhap so hang va so cot: ";
    if (!(cin >> so_hang >> so_cot)) return 0;
#endif
...
#else
    cerr << "Nhap ma tran (su dung 'D' cho diem dau/cuoi, '1' cho tram dung chan, '0' cho o trong, '#' la tuong):\n";
    for (int i = 0; i < so_hang; ++i) cin >> luoi[i];
#endif

    // Kiểm tra độ dài hàng nhập
    ...
    if (so_tram == 0) {
        // Trường hợp không có trạm: BFS trực tiếp từ start đến đích
    ...
    // Danh sách các điểm quan trọng: 0 = start, 1.. = tram
    ...
    // Ban do khoang cach (ma tran so_diem_xu_ly x so_diem_xu_ly)
